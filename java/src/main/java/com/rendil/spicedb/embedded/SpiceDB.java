 ...
 *
 * <p>Build shared/c first: {@code mise run shared-c-build} or {@code cd shared/c && CGO_ENABLED=1 go
 * build -buildmode=c-shared -o libspicedb.dylib .}
 */
interface SpiceDB extends Library {
    ...
    static String findLibraryPath() {
        ...
        for (String rel : new String[] {"shared/c", "../shared/c", "java/../shared/c"}) {
            java.nio.file.Path path = java.nio.file.Path.of(base).resolve(rel).resolve(libName).normalize();
    ...
    /**
     * Start a new SpiceDB instance. Returns JSON: {"success": true, "data": {"handle": N, "socket_path":
     * "..."}}
     */
